---
name: run-experiment
description: 実データセットで apriori_window_suite を実行し、結果を記録・比較する（Phase 3）。新しいデータセットでの初回実験 / パラメータ探索 / 複数設定の比較実験に使う。
---
...
- **失敗例1: ファイルパスが見つからない** / 回避策: `dir` には常に絶対パスを使用する
- **失敗例2: パターンが 0 件** / 回避策: min_support を半分にして再実行する
- **失敗例3: 実行がタイムアウト** / 回避策: `--release` ビルドを使い、min_support を 2 倍にして再実行する
- **失敗例4: Python と Rust の件数が異なる** / 回避策: 差分が 5% 未満なら許容し、大きければ `impl-feature` スキルでバグ修正を行う

---

## 6. Evaluation

- **合格ライン**: patterns.csv が生成され、件数が合理的（1〜10万件）+ log.md に記録済み
- **重大欠陥（即NG）**:
