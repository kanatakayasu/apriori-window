...
```bash
python -m comparative_methods.runner.run_method \
...
python -m comparative_methods.runner.preprocess_inputs \
  --input apriori_window_suite/data/sample_basket.txt \
  --input-format basket \
  --out-dir comparative_methods/inputs \
  --stem sample_basket
```

```bash
# 集計（複数結果JSON -> CSV）
python -m comparative_methods.runner.aggregate_stage_a \
  --results-glob 'comparative_methods/results/*.json' \
  --gt-dir dataset/synthetic/ground_truth \
  --out-csv comparative_methods/results/stage_a_summary.csv
```

```bash
# Stage A 比較手法を一括実行（入力1ファイル）
python -m comparative_methods.runner.run_stage_a_suite \
  --input-basket apriori_window_suite/data/sample_basket.txt \
  --out-dir comparative_methods/results \
  --backend auto
```
