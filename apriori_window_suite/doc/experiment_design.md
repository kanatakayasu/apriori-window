...
**従来アルゴリズムの制約**:
従来は 1 トランザクション = 1 バスケット（固定）であるため、**同一時間帯に複数の購買が発生するデータを扱えない**。

複数ユーザーの密集パタンを取得するには、同一時間帯の全購買を 1 トランザクションに集約する必要があるが、その場合は独立した複数の購買（バスケット）がひとつのトランザクションに混入し、**バスケット間のアイテムが誤って「共起」とみなされる（偽共起）**。

```
同一時間帯の購買（3件のバスケット）:
  ユーザーA の購買（バスケット B1）: {牛乳, パン}
  ユーザーB の購買（バスケット B2）: {ビール, ポテチ}
  ユーザーA の別の購買（バスケット B3）: {シャンプー}

→ 従来法（1トランザクション=1バスケットの制約で全統合）:
  トランザクション T = {牛乳, パン, ビール, ポテチ, シャンプー}
  牛乳-ビール, 牛乳-ポテチ, パン-ビール … すべてが「共起」とみなされる（偽共起）

→ Phase 1（バスケット構造を保持）:
  T = {牛乳 パン | ビール ポテチ | シャンプー}
  牛乳-パン のみが「共起」（B1 内）、ビール-ポテチ のみが「共起」（B2 内）
```

**本質**: 偽共起の源泉は「同一時間帯に複数バスケットが存在すること」であり、
ユーザーの違い・同一ユーザーの複数来店などはその具体的なシナリオの一例に過ぎない。
Phase 1 は、トランザクションが複数バスケットを持てる構造に拡張することでこれを解決する。
...
1 トランザクション = 1 時間帯。その時間帯に発生した独立した購買イベントをバスケットとして集約する。

```
for t in 1..N_transactions:
    N ~ Poisson(λ_baskets)  # この時間帯に発生した購買イベント数（バスケット数）
    N = max(N, 1)

    for n in 1..N:  # 各バスケット（独立した購買イベント）
        K ~ Poisson(λ_basket_size)  # バスケット内アイテム数
        K = max(K, 1)

        # メインカテゴリを選択（各購買は独立）
        c_main ~ Categorical(category_popularity)
...
**偽共起の発生**: N ≥ 2 のとき、異なるバスケットのアイテムが同一トランザクション内で
混在するため、従来法ではバスケット間の偽共起が検出される。
λ_baskets が大きいほど 1 トランザクションあたりのバスケット数が増え、偽共起の発生率が上昇する。
...
#### 実験 A1：λ_baskets の影響（RQ1 の主実験）

λ_baskets（1 時間帯あたりの平均バスケット数）を変化させ、偽共起の発生率が
λ_baskets に比例して増加することを示す。λ_baskets = 1 のとき偽共起は発生しない（ベースライン）。
...
| 変化パラメータ | 設定値 |
|--------------|-------|
| `λ_baskets` | 1.0, 1.5, 2.0, 3.0, 5.0 |

**期待される結果**:

```
λ_baskets=1.0: SPR = 0（各時間帯に購買1件 → 偽共起なし。従来法 = Phase 1）
λ_baskets=2.0: SPR > 0（複数購買の混入で偽共起が発生し始める）
λ_baskets=5.0: SPR が顕著に上昇（従来法の問題が明確化）
Phase 1:        SPR = 0 を λ_baskets に関わらず常に維持
```

#### 実験 A2：カテゴリ構造の影響（RQ1 の補足実験）

λ_baskets=2.0 に固定し、カテゴリ数 G を変化させる。
G が小さい（カテゴリ境界が曖昧）ほど偽共起の「検出への影響」が大きいことを示す。
...
λ_baskets=2.0、G=10 に固定し、データ規模を変化させる。
...
**バスケットの定義**: `InvoiceNo`（1回の購買イベント）
**トランザクションの定義**: `日付`（同一日付の全購買を1トランザクションに集約）

複数ユーザーの密集パタンを捉えるため、トランザクションは顧客単位ではなく**時間単位（日）** で集約する。
同一日に発生した複数のInvoiceNo（異なる顧客・同一顧客の複数購買）が複数バスケットを形成する。

```
前処理ステップ:
1. キャンセル行を除外（InvoiceNo が 'C' で始まる行）
2. Quantity ≤ 0 の行を除外
3. InvoiceDate から 日付（date）を抽出
4. グループキー = date → トランザクションID を採番（日付順の連番）
5. InvoiceNo をバスケットID として採番（グローバル連番）
6. バスケット構造付き入力形式に変換:
   date=2010-01-05, baskets=[
     InvoiceNo=536365（CustomerID=12345）→ {85123A, 71053, ...}
     InvoiceNo=536366（CustomerID=17850）→ {84406B, ...}
     InvoiceNo=536367（CustomerID=12345）→ {22752, ...}  ← 同一顧客の別購買も別バスケット
     ...
   ]
   → 出力行: "85123A 71053 | 84406B | 22752 ..."
7. タイムスタンプ = トランザクションIDの連番（整数）
```

**注意点**:
- 1日あたりの InvoiceNo 数（≈バスケット数）が多いほど偽共起の源泉となる
- CustomerID 欠損行は除外しない（購買イベントとしては有効なため）
- 1日あたりのバスケット数が実験 A1 の λ_baskets に相当する実測値となる
...
Step 3  実験 A1（λ_baskets sweep, N=10K）の実行と SPR 計測
...
| UCI の 1 日あたりバスケット数 | 前処理後に実測して確認する | B1 実装前 |
...
| 実験 | 論文セクション（想定） | 主張 |
|------|---------------------|------|
| A1 | 提案手法の評価（合成） | Phase 1 は λ_baskets に関わらず偽共起を排除する |
