...
- **目的**: Apriori + スライディングウィンドウで密集アイテムセット区間を検出し、外部イベントとの時間的関係を列挙する
- **主要成果物**: `apriori_window_suite/` クレート（Rust + Python 二重実装）
- **現在のフェーズ**: Phase 1 完了 → Phase 2 パイプライン再設計中（4 段パイプライン）
- **Phase 2 設計**: `doc/temporal_relation_pipeline.md` — MI 事前フィルタ + Sweep Line + 置換検定
...
apriori_window/
  ...
  apriori_window_original/         ← 旧版リファレンス実装（workspace exclude）
  ...
  doc/
    temporal_relation_pipeline.md  ← Phase 2 パイプライン設計（4 段: MI→Sweep→置換検定）
  tools/
...
| Python パス | `event_correlator.py` は `sys.path.insert(0, parent)` で Phase 1 モジュールを import |
| 時間的関係 | 6種 Allen 関係: DFE / EFD / DCE / ECD / DOE / EOD（4 段パイプラインで抽出: `doc/temporal_relation_pipeline.md`） |
