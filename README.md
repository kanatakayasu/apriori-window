...
Apriori + スライディングウィンドウによる密集アイテムセット区間検出と、外部イベントとの時間的関係列挙のための研究開発リポジトリ。

## 概要

Apriori とスライディングウィンドウを組み合わせ、頻出アイテムセットが密集して出現する時間区間を検出する。さらに、検出された密集区間と外部イベントの時間的関係を自動的に列挙することで、データマイニングにおけるイベント相関分析を支援する。

- **Phase 1**: Apriori + スライディングウィンドウで密集アイテムセット区間を検出
