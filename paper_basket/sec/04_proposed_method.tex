...
\begin{figure}[t]
    \centering
    % ユーザー作成予定: 従来法 vs Phase 1 の共起カウント比較（詳細は本文末の図案を参照）
    \includegraphics[width=\linewidth]{fig/conventional_vs_phase1.pdf}
    \caption{従来法（flatten）と提案手法（Phase 1）の処理の違い．
    従来法では全バスケットを統合するため，異なるバスケット由来のアイテムが
    共起としてカウントされる（赤矢印が偽共起の例）．
    提案手法はバスケット単位で共起を評価するため偽共起が発生しない．}
...
提案手法（Phase 1）の出力は従来 Apriori-window の出力と一致する．
\end{proposition}
