...
\begin{figure}[t]
    \centering
    % ユーザー作成予定: 表形式の具体例（詳細は本文末の図案を参照）
    \includegraphics[width=\linewidth]{fig/example_table.pdf}
    \caption{バスケット構造付きトランザクションデータの例（5 トランザクション）．
    各行が 1 日分のデータであり，同一日に複数の購買イベント（バスケット）が存在する．
    バスケット内共起に基づくと，(牛乳, ビール) はどのバスケットにも共起せず，
    $\text{true\_support} = 0$ となる．
    従来法（トランザクション内共起）では $\text{txn\_support} = 3/5 = 0.6$ となり，
    偽共起が発生する．}
