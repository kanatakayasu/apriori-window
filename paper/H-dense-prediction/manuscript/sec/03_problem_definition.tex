...
\paragraph{パターン長に関する前提}
本研究では、予測モデル（DIOP）への入力としてペアワイズ（$L = |X| = 2$）の
アイテムセットから生成される密集区間を主な対象とする。
$L=2$ は密集区間マイニングの標準的な設計前提であり~\cite{lancaster2018surges}、
以下の理由から予測問題に適している：
(1) 密集区間の発生頻度が十分に高く、点過程モデルのパラメータ推定に
必要な標本サイズを確保しやすい、
(2) $L$ が大きくなるとサポートの低下により密集区間数が減少し、
Hawkes過程の推定精度が劣化する。
$L \geq 3$ の場合の予測精度への影響は今後の検討課題である
（\S\ref{sec:conclusion}参照）。
...
\begin{itemize}
  ...
  \item \textbf{対数尤度比}（LLR）：$\mathrm{LLR} = \ell_{\text{Hawkes}} - \ell_{\text{Poisson}}$。
    ...
    カイ二乗検定で有意（$p < 0.10$）の場合にHawkesモデルが優位と判定する。
    有意水準を $\alpha = 0.10$ に設定するのは、
    密集区間数が少ない設定における検出力を確保するためである。
\end{itemize}
