...
\subsection{モデルの前提条件と制約}\label{subsec:constraints}

提案パイプラインは以下の前提条件に依存する。実データへの適用時には各条件の妥当性を
確認する必要がある。

\begin{enumerate}
  \item \textbf{Hawkes過程の定常性仮定}：
    $\alpha < 1$ の条件下で過程が定常であることを仮定する。
    非定常な環境（例：長期トレンドや季節性が顕著な場合）では、
    時間区間の分割やベースライン強度の時変化拡張が必要となる。
  \item \textbf{Weibullモデルの形状パラメータ依存性}：
    持続時間予測の精度は形状パラメータ $k$ の推定精度に依存する。
    密集区間数が少ない場合（$K < 20$）、$k$ の信頼区間が広くなり
    予測区間の実用的な精度が低下する可能性がある。
  \item \textbf{密集区間定義のウィンドウサイズ依存性}：
    密集区間の検出結果はウィンドウサイズ $W$ と閾値 $\theta$ の設定に依存する。
    これらのハイパーパラメータの選択が予測モデルの入力（密集区間列）を
    決定するため、$W$ と $\theta$ の感度分析が実運用では重要である。
  \item \textbf{パターン長の制約}：
    $L=2$（ペアワイズ）パターンを前提とする
    （\S\ref{sec:problem}参照）。$L \geq 3$ では密集区間数の減少により
    点過程モデルの推定が不安定になる傾向がある。
\end{enumerate}
