...
\begin{theorem}[予測の一致性]
  $K \to \infty$ のとき、
  最尤推定量 $(\hat{\mu}, \hat{\alpha}, \hat{\beta})$ は
  真のパラメータ $(\mu^*, \alpha^*, \beta^*)$ に確率収束する。
  したがって、thinningアルゴリズムによる予測分布は
  真の条件付き強度に基づく分布に収束する。
\end{theorem}

\begin{proof}
  Hawkes過程の最尤推定量の一致性は点過程の漸近理論により保証される~\cite{daley2003introduction}。
  thinningアルゴリズムは正確なシミュレーション手法であるため、
  パラメータの一致性から予測分布の一致性が従う。
\end{proof}
