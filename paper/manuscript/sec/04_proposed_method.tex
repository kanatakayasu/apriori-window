...
\textbf{変化量}: 式~(\ref{eq:magnitude})のレベルシフト量を
以下の平方根正規化により評価する:
\begin{equation}
\text{mag}(\tau) = \frac{|\bar{s}^{+}(\tau) - \bar{s}^{-}(\tau)|}{\sqrt{\max(1,\; \bar{s}^{-}(\tau))}}
\label{eq:mag_sqrt}
\end{equation}
絶対変化量 $|\bar{s}^{+} - \bar{s}^{-}|$ はベースラインサポートの水準に依存するため，
ベースラインサポートが高いパターンほど大きな値を取るバイアスを持つ．
平方根正規化は，異なるベースライン水準のパターン間でスコアの
公平な比較を可能にする．
完全正規化（$\div \bar{s}^{-}$）は低ベースラインパターンのスコアを過剰に増幅するが，
平方根正規化はこの過増幅を避けつつ正規化の効果を保持する（EX2で検証）．
データ特性に応じて正規化なし（絶対変化量）に切り替えることも可能である．
