...
\paragraph{前提条件: $L = 2$（2-アイテムセット設計）}
本パイプラインは最大パターン長 $L = 2$ を前提として設計されている．
これは，Step 5 のアイテム重複排除（命題~\ref{prop:dedup}）が
2-アイテムセット間のアイテム共有構造に基づいて正当化されること，
および実応用においてペアワイズ共起が最も解釈可能な単位であることによる．
$L \geq 3$ への拡張は，重複排除における連結成分の構造が複雑化するため，
今後の課題とする．
...
対応する $p$ 値間に正の依存性が誘導され，PRDS条件は実務的に妥当と考えられる．
ただし厳密な証明は本設定の複雑さから困難であるため，
FDR制御は実務的妥当性に基づいて適用する．
\end{remark}
...
例えば，イベントが $\{42, 87\}$ をブーストし真のパターンが $(42, 87)$ の場合，
$(42, 155)$, $(78, 87)$, $(42, 133)$ などの副次パターンも
