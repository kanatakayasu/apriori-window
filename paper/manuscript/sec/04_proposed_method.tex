...
\begin{remark}[置換検定の正当性]\label{prop:perm_exact}
帰無仮説 $H_0$ のもとでは，観測スコア $A_{\text{obs}}$ と
$B$ 個の置換スコア $A_{\text{perm}}^{(1)}, \ldots, A_{\text{perm}}^{(B)}$ は交換可能（exchangeable）である．
これは円形シフトが帰無仮説下で時系列の自己相関構造を保存するという
サロゲートデータ法~\cite{theiler1992testing}の標準的性質に基づく．
...
本設定では，パターン $P_1 \cap P_2 \neq \emptyset$ のとき，
$P_1 \subseteq d_t \Rightarrow P_2$ の出現確率が
$P_1 \not\subseteq d_t$ の場合以上となる（共有アイテムが出現条件を緩和するため）．
したがって $s_{P_1}(t)$ と $s_{P_2}(t)$ は正の相関を持ち，
これがスコア $A_{\text{obs}}$ および $p$ 値に単調に伝播する．
帰無仮説が真であるパターンの $p$ 値集合に対して，
帰無仮説が偽であるパターンの $p$ 値が小さくなる方向に
条件付き期待値が単調増加するため，PRDS条件が満たされる．
\end{remark}
...
\begin{proposition}[重複排除の正当性（一般 $k$-itemset）]\label{prop:dedup}
真のパターン $P^* = \{i_1, \ldots, i_k\}$（$|P^*| = k \geq 2$）が
イベント $e$ によりブースト確率 $\beta$ でサポートを増加させられているとする．
副次パターン $P' = (P^* \setminus \{i_r\}) \cup \{j\}$（$j \notin P^*$）において，
$j$ のベースライン出現確率を $p_{\text{base}}$ とする．
$\beta > p_{\text{base}}$ であれば，
$\text{mag}(\tau, P^*) > \text{mag}(\tau, P')$ が成立し，
$P^*$ と $P'$ はアイテム共有（$k-1$ 個）により同一連結成分に属するため，
$P^*$ が保持され $P'$ は除去される．
\end{proposition}

\begin{proof}
アイテムの出現が独立であると仮定する．
イベント $e$ は $P^*$ の全 $k$ アイテムの出現確率を $\beta$ だけ増加させる．
ウィンドウ $W$ 内での $P^*$ のサポート増分は
\begin{equation*}
\Delta s_{P^*} \propto W \cdot \beta^k
\end{equation*}
である（全 $k$ アイテムがブーストされる）．
一方，副次パターン $P'$ では $k-1$ アイテムがブーストされ，
$j$ は独立にベースライン確率 $p_{\text{base}}$ で出現するため，
\begin{equation*}
\Delta s_{P'} \propto W \cdot \beta^{k-1} \cdot p_{\text{base}}
\end{equation*}
である．$\beta > p_{\text{base}}$ のとき
$\beta^k > \beta^{k-1} \cdot p_{\text{base}}$ が成立し，
$\text{mag}(\tau, P^*) > \text{mag}(\tau, P')$ となる．

$P^*$ と $P'$ は $k-1$ 個のアイテムを共有するため，
アイテム共有グラフ上で同一連結成分に属する．
したがって，各連結成分内でスコア最大のパターンを選択する
重複排除ステップにおいて $P^*$ が保持され $P'$ は除去される．

なお，副次パターンが $P^*$ と $\ell < k-1$ 個のアイテムのみを共有する場合でも，
中間パターン（$\ell+1, \ell+2, \ldots, k-1$ 個共有）を経由する
推移的連結により同一成分に属する．
この場合のサポート増分は $\Delta s_{P'} \propto W \cdot \beta^\ell \cdot p_{\text{base}}^{k-\ell}$
であり，$\beta > p_{\text{base}}$ のもとで $P^*$ がスコア最大となる条件は維持される．
\end{proof}
