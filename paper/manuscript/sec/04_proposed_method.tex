...
\paragraph{設計前提: $|P|=2$（2-アイテムセット）}
本パイプラインは 2-アイテムセット（ペアパターン）を対象に設計されている．
Step 5 の重複排除（命題~\ref{prop:dedup}）における独立出現モデルは
$|P|=2$ を前提としており，3 以上のアイテムからなるパターンへの拡張は
今後の課題である（\ref{sec:conclusion}節）．
Phase 1 の最大パターン長パラメータを $L=2$ に設定して使用する．
...
\begin{remark}[PRDS 条件についての議論]\label{rem:prds}
...
対応する $p$ 値間に正の依存性が誘導され，PRDS条件が成立する根拠となる．
ただし，円形シフト帰無分布の離散性や副次パターンの存在により
理論的保証の厳密な適用には注意を要する．
\ref{sec:exp_null}節の帰無実験により，実務的な FDR 制御を実験的に確認する．
\end{remark}
...
2-アイテムセット $P=\{i_1,i_2\}$ のサポートは各アイテムの出現確率の積に比例する．

真のパターン $P^* = \{i_1, i_2\}$（$|P^*|=2$）がイベント $e$ によりブースト確率 $\beta$ で
サポートを増加させられているとする．
副次パターン $P' = \{i_1, j\}$（$j \notin P^*$）において，
