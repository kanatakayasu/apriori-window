...
\caption{(a) パイプラインの全体構成．入力（パターン集合，密集区間，イベント）から5ステップを経て有意な帰属を出力する．(b) 帰属スコアリングの例示．サポート時系列上の変化点 $\tau_{\uparrow}, \tau_{\downarrow}$ とイベント区間の関係から，近接度（prox），方向整合性（dir），変化量（mag）を算出する．}
\label{fig:pipeline}
\end{figure}
...
対応する $p$ 値間に正の依存性が誘導され，PRDS条件は自然に満たされる．
さらに，円形シフトは全イベントを同一オフセットで一斉にずらすため，
同一パターンに対する異なるイベントの $p$ 値も同じ置換標本を共有し，
正の依存性を持つ．したがってイベント間の依存構造もPRDS条件を満たす．
\end{remark}
