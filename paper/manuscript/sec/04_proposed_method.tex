...
\textbf{変化量}: 式~(\ref{eq:magnitude})のレベルシフト量を評価する．
3種類の正規化方式を提供する:
\begin{align}
\text{mag}_{\text{none}}(\tau) &= |\bar{s}^{+}(\tau) - \bar{s}^{-}(\tau)| \label{eq:mag_none}\\
\text{mag}_{\text{sqrt}}(\tau) &= \frac{|\bar{s}^{+}(\tau) - \bar{s}^{-}(\tau)|}{\sqrt{\max(1,\; \bar{s}^{-}(\tau))}} \label{eq:mag_sqrt}\\
\text{mag}_{\text{full}}(\tau) &= \frac{|\bar{s}^{+}(\tau) - \bar{s}^{-}(\tau)|}{\max(1,\; \bar{s}^{-}(\tau))} \label{eq:mag_full}
\end{align}
絶対変化量（式~(\ref{eq:mag_none})）は全パターンが同程度のベースラインサポートを
持つ場合に最適であるが，ベースラインが高いパターンほど大きな値を取るバイアスがある．
平方根正規化（式~(\ref{eq:mag_sqrt})）は異なるベースライン水準間での
公平な比較を可能にし，Zipf的な頻度分布を持つデータに適する．
完全正規化（式~(\ref{eq:mag_full})）は低ベースラインパターンのスコアを
過剰に増幅する傾向がある．
データのアイテム頻度分布に応じた最適な方式の選択については
EX1で検証する．
