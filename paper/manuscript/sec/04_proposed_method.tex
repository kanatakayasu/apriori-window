...
2アイテムセットの簡約独立出現モデル下で証明されており，
またアイテム共有グラフにおいて2アイテム中1アイテムの共有は
50\%の重複に相当し，統合の妥当な根拠となる．
$|P| \geq 3$ では，ヘッドアイテム（高頻度アイテム）1つの共有だけで
無関係なパターンが同一クラスタに過剰統合される（over-merging）リスクが増大し，
...
パラメータ感度実験（EX3，付録）では閾値交差とCUSUMが経験的に同等の検出性能を示すことを確認した．
...
本ステップは $|P| = 2$ を前提とする（本節冒頭の前提条件を参照）．
...
\begin{proposition}[重複排除の正当性（簡約モデル下）]\label{prop:dedup}
以下の簡約的な独立出現モデルを仮定する:
...
\begin{proof}[証明の概略]
上記の独立出現仮定のもとでは，
イベント $e$ の影響下で $i_1, i_2$ の各出現確率が $\beta$ だけ増加する．
...
EX6（Correlated条件）の精度低下はこの限界を反映している．
\end{proof}
