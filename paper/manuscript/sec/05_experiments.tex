...
3つの正規化方式を比較する:
\begin{itemize}
\item \textbf{正規化なし}（式~(\ref{eq:mag_none})）: 絶対変化量
...
条件 & なし & $\sqrt{}$ & 完全 \\
\midrule
一様（$\beta=0.3$） & \textbf{0.85} & 0.83 & 0.79 \\
Zipf-1.0 & 0.73 & \textbf{0.77} & 0.75 \\
Zipf-1.5 & 0.76 & \textbf{0.79} & 0.77 \\
\bottomrule
\end{tabular}
\end{table}

一様分布では\textbf{正規化なし}がF1 = 0.85で最も高い精度を達成する．
全パターンのベースラインサポートが同水準であるため，
正規化はスコアの弁別力を低下させるのみである
（$\sqrt{}$: 0.83，完全: 0.79と段階的に劣化）．

Zipf分布では平方根正規化が一貫して最良の結果を示す．
Zipf-1.0ではF1 = 0.77（正規化なし0.73比 +5\%），
Zipf-1.5でもF1 = 0.79（正規化なし0.76比 +4\%）を達成する．
高頻度アイテムペアの自然変動によるスコア膨張が正規化により抑制され，
真の信号が相対的に浮上するためである．
完全正規化はZipf-1.0でF1 = 0.75，Zipf-1.5で0.77と
平方根正規化をわずかに下回る．
...
従うケースが多く~\cite{fournier2017survey}，
頻度分布が不明な場合は平方根正規化が安全な選択肢である．
以降の実験では，データ特性に応じた最適な正規化方式を使用する．
...
実世界のトランザクションデータはアイテム頻度がZipf分布に
従うケースが多いため，EX2以降ではZipf分布（$\alpha=1.0$）データに
平方根正規化を適用して評価する．
...
\begin{table}[t]
\caption{EX2a 結果: 信号強度別の帰属精度（5 seeds平均，Zipf-1.0，$\sqrt{}$正規化）}
\label{tab:ex2a}
\centering
\begin{tabular}{ccccc}
\toprule
...
4つの構造条件を評価した結果を表~\ref{tab:ex2b}に示す．

\begin{table}[t]
\caption{EX2b 結果: 構造条件別精度（5 seeds平均，Zipf-1.0，$\sqrt{}$正規化）}
\label{tab:ex2b}
\centering
\begin{tabular}{lcccc}
\toprule
...
EX2と同様にZipf分布データ（$\sqrt{}$正規化）を使用する．
...
\begin{table}[t]
\caption{EX3a 結果: シナリオ別アブレーション（5 seeds平均F1，Zipf-1.0，$\sqrt{}$正規化）}
\label{tab:ex3a}
\centering
\begin{tabular}{lcc}
\toprule
...
EX2のデータ（$\beta = 0.3$, CONFOUND, DENSE; Zipf-1.0, $\sqrt{}$正規化）を使用し，
各条件 $\times$ 5 seedsで評価した（表~\ref{tab:ex3b}）．

\begin{table}[t]
\caption{EX3b 結果: ナイーブベースラインとの段階的比較（5 seeds平均，Zipf-1.0，$\sqrt{}$正規化）}
\label{tab:ex3b}
\centering
