...
\ENSURE 因果効果 $\hat{\tau}$，p値 $p$，帰属スコア $\mathrm{Score}$
...
\STATE $\mathrm{Score}(P^*, e) \leftarrow \mathrm{prox}(t_0, t_e) \times \mathrm{mag}(P^*, t_0)$ を計算（\Cref{def:attribution-score}）
\RETURN $\hat{\tau}$, $p$, $\mathrm{Score}$
\end{algorithmic}
\end{algorithm}
