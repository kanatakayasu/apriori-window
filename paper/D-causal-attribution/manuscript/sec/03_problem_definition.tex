...
本研究では，因果帰属の基本単位として2アイテムパターン（$|P|=2$，以下 $L=2$）に焦点を当てる．
$L=2$ は因果的共起の最小構成要素であり，以下の理論的・実用的利点を持つ：
(1) 交絡制御の容易性：アイテム数が少ないほど，介入がサポートに与える直接効果と交絡による間接効果の分離が明確になる；
(2) ドナープール構築の単純性：$L=2$ ではアイテム非共有基準（\Cref{def:item-disjoint}）の適用が容易であり，十分なドナー数を確保しやすい；
(3) 解釈可能性：2アイテム間の因果的共起変動は，実務者にとって直感的に理解しやすい~\cite{lancaster2018surges}．
...
\label{def:attribution-score}
外部イベント $e$（発生時刻 $t_e$）に対する処置パターン $P^*$ の\emph{帰属スコア}は，時間的近接度と効果量の積として定義される：
\begin{equation}
    \mathrm{Score}(P^*, e) = \mathrm{prox}(t_0, t_e) \times \mathrm{mag}(P^*, t_0)
\end{equation}
ここで，$\mathrm{prox}(t_0, t_e) = \exp(-|t_0 - t_e| / \lambda)$（$\lambda$ は減衰パラメータ）は介入時刻とイベント発生時刻の時間的近接度，$\mathrm{mag}(P^*, t_0) = |\mathcal{T}| / (T - t_0 + 1)$ は正規化された累積効果量である．
因果推論において，帰属の根拠は(1) 効果の大きさ（mag）と(2) 時間的近接性（prox）の二要素に集約される．
方向性（dir）は因果の向きとは異なる概念であるため，帰属スコアには含めない．
\end{definition}
...
$p < 0.10$ を達成するには，少なくとも $J \geq 9$ 個のドナーが必要である．
\end{corollary}
