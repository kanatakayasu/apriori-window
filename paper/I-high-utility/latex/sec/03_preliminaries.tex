...
\label{def:udi}
アイテムセット$X$は、パラメータ$(W, \theta_f, \theta_u)$に対して、すべての$l \in [s, e]$で以下を満たすとき、\emph{効用密集区間}$[s, e]$を持つ：
\begin{enumerate}
...
\textbf{問題定式化。}
効用トランザクションデータベース$\mathcal{D}$、ウィンドウサイズ$W$、頻度閾値$\theta_f$、効用閾値$\theta_u$、最大アイテムセット長$k_{\max}$が与えられたとき、$[s, e]$が$X$の極大効用密集区間であり$|X| \leq k_{\max}$を満たすすべてのペア$(X, [s, e])$を求めよ。
