% ==============================================================
\section{PRDS Condition Justification}\label{app:prds}
% ==============================================================

For the BH procedure to control FDR for non-independent test statistics,
the $p$-values must satisfy the positive regression dependency on a subset (PRDS)
condition~\cite{benjamini2001control}.
We do not provide a formal proof here, but the positive correlation between support
time series of patterns sharing items induces positive dependence among
corresponding $p$-values, serving as the practical rationale for applying PRDS.
...
The deduplication step has complexity $O(n \cdot k)$ per event
($n$: significant attributions, $k$: max pattern length),
negligible compared to permutation testing.
...
Claim~\ref{prop:dedup} (validity of deduplication).
...
of Claim~\ref{prop:dedup} is relaxed in real data;
