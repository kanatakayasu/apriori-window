...
\title{パラメータフリー密集区間マイニング：\\適応的ウィンドウサイズによるTop-$k$パターン発見}
...
本研究では、これらのパラメータを直感的な単一パラメータ$k$（所望のパターン数）に
置き換える\emph{パラメータフリー}フレームワークを提案する。
本手法は3つの新しい概念を導入する：
...
本論文では、これらのアイデアを橋渡しして、密集区間マイニングのための
パラメータフリーフレームワークを提案する。本研究の貢献は以下の通りである：
...
\item グリッドサーチとの品質同等性、効果的な枝刈り、ほぼ線形のスケーラビリティ、
  およびパラメータ感度の排除を実証する包括的な実験を提示する
...
\subsection{マルチスケールアプローチの利点}

二分スケール階層は2つの主要な利点を提供する：
(1) $[W_0, 2^L W_0]$内の任意のスケールで密集であるパターンが、
ユーザが正しいウィンドウサイズを推測する必要なく捕捉される；
(2) スケール空間密集リッジは、密度がスケールにわたってロバストである
パターンを特定し、単一スケール分析よりも真の時間的集中のより強い証拠を提供する。
...
本論文では、従来の$(W, \theta)$パラメータを直感的な単一パラメータ$k$に置き換える、
密集区間マイニングのためのパラメータフリーフレームワークを提示した。
...
実験では、網羅的グリッドサーチとの品質同等性、効果的な枝刈り、
ほぼ線形のスケーラビリティ、およびロバストなパラメータ非感受性を実証した。
