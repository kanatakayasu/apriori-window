...
\noindent
\textbf{記法}: $\mathcal{I} = \{i_1, i_2, \ldots, i_m\}$ を有限のアイテム全集合とする．
$\mathcal{I}$ の任意の部分集合 $P \subseteq \mathcal{I}$ をアイテムセットと呼び，
$|P|$ をその長さとする．
また，バスケット $B$ は 1 回の独立した購買イベントを表す
...
\noindent
\textbf{データ列}: バスケット構造付きトランザクションデータ
$D = [d_1, d_2, \ldots, d_N]$ は，タイムスタンプの昇順に並べられた
トランザクション列である．$N = |D|$ をデータ長と呼ぶ．
...
\noindent
\textbf{区間記法}: 区間 $I = [ts_i, ts_j]$（$1 \leq i \leq j \leq N$）に対して，
$D_I = [d_i, d_{i+1}, \ldots, d_j]$ を区間 $I$ 内のトランザクション列とし，
$n(D_I) = j - i + 1$ をその長さとする．

\noindent
\textbf{サポート関数}: 区間 $I$ におけるアイテムセット $P$ の
バスケット内サポート（true support）を
\begin{equation}
...
\end{equation}
とし，トランザクション内サポート（txn support）を
\begin{equation}
...
\noindent
\textbf{評価指標 (SPR)}:
最小サポート $\theta$ に対して，txn 頻出パターン集合を
...
とする．偽共起パターン率（SPR: Spurious Pattern Rate）を
\begin{equation}
...
\label{prop:spr_zero}
提案手法が出力するすべての密集パターンについて，SPR = 0 が成立する．
\end{proposition}
