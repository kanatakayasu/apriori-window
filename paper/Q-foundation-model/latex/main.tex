...
\subsection{AUC 0.497 の詳細分析: 情報的負の結果}

AUC 0.497（ランダム分類器 0.5 とほぼ同等）は一見して失敗に見えるが、
本節ではこの結果が提供する情報的価値を分析する。

\textbf{原因の特定}: DIP精度の低さは以下の因果連鎖で説明される。
(1) アイテム埋め込み $\mathbf{e}_i$ は Xavier 一様分布で初期化され、
アイテム間の意味的関係を反映しない。
(2) 平均プーリングにより構成されるトランザクション埋め込み $\phi(T_t, t)$ は、
密集区間内外で体系的な差異を持たない
（E5: 事前学習前後の分離度が 0.270 で変化なし）。
(3) 固定エンコーダの出力 $\mathbf{h}_t^{(L)}$ は、
ランダム初期化の注意重みによる「平均化」効果で
位置間の差異がさらに平滑化される。
(4) DIPヘッドの線形分類器は、入力表現に密集区間の情報が
含まれていないため、分類境界を学習できない。

\textbf{他分野との対比}: この段階的な原因分析は、
基盤モデル研究における初期プロトタイプの典型的パターンと一致する。
TabTransformer~\cite{huang2020tabtransformer}の初期版も
事前学習なしでは勾配ブースティングに劣後し、
半教師あり事前学習の導入で初めて優位性を示した。
BERT4Rec~\cite{sun2019bert4rec}もマスク予測の事前学習が
推薦精度に決定的な影響を与えることを報告している。
すなわち、AUC 0.497 は端到端学習の\textbf{必要性}を実証する
正の証拠として解釈すべきである。

\textbf{改善の定量的予測}: 端到端学習によりエンコーダが更新された場合、
MTM損失の大幅な削減（4.1\% $\to$ 推定 30--50\%）が期待され、
それに伴い密集区間内外のトランザクション表現の分離度が
0.270 から 0.6 以上に改善すると予測される。
線形分類器の性能は入力表現の分離度と強く相関するため、
DIP AUC は 0.75--0.85 程度に到達する可能性がある。
この予測は、類似のマスク予測事前学習を用いた
PatchTST~\cite{nie2023patchtst}の報告（事前学習による
予測精度 20--40\% 改善）と整合的である。

\subsection{集合ベースエンコーディングのアーキテクチャ的新規性}

本手法の中心的なアーキテクチャ貢献は、
\textbf{可変長集合値トランザクション}を Transformer の入力として
適切に表現する埋め込み設計にある。
既存の Transformer 応用（TabTransformer、BERT4Rec、PatchTST 等）は
固定長ベクトル、単一アイテム列、または連続値時系列を入力とするが、
トランザクションデータは各時点が\textbf{順序のない可変長部分集合}であり、
以下の 3 つの設計課題を提起する。

\textbf{課題 1: 置換不変性}. トランザクション $T_t = \{i_1, i_2, \ldots, i_k\}$ の
埋め込みはアイテムの順序に依存してはならない。
本手法の平均プーリング $\frac{1}{|T_t|}\sum_{i \in T_t} \mathbf{e}_i$ は
最も単純な置換不変集約であるが、
DeepSets~\cite{zaheer2017deep}の理論が保証するように、
任意の置換不変関数を近似可能な $\rho(\sum_i \phi(x_i))$ 型への拡張が可能である。

\textbf{課題 2: 可変濃度}. $|T_t|$ はトランザクションごとに異なるため、
固定長入力を前提とする TabTransformer とは異なる正規化戦略が必要である。
本手法の $1/|T_t|$ による正規化は、大きなトランザクションの
ノルムが過大になることを防ぐ。

\textbf{課題 3: 密集性の時間的局在}. 密集区間は局所的なサポート集中であり、
グローバルな頻出性とは異なる。Transformer の自己注意は
全位置間の依存関係を計算するため、密集区間の局所的パターンを
捕捉する能力を持つが、それには適切な事前学習による
表現の条件付けが不可欠である（AUC 0.497 の分析が示す通り）。

\subsection{PyTorch 実装ロードマップ}

提案アーキテクチャの PyTorch 実装を以下の 3 フェーズで計画する。

\textbf{Phase 1: コア実装（推定 2 週間）}。
\texttt{torch.nn.Module} として以下のコンポーネントを実装する:
(a) \texttt{TransactionEmbedding}: アイテム埋め込みテーブル
（\texttt{nn.Embedding}）+ 平均プーリング + 正弦波位置符号化、
(b) \texttt{PreLNTransformerBlock}: \texttt{nn.MultiheadAttention}
+ Pre-LN + FFN + 残差接続 + ドロップアウト、
(c) \texttt{MTMHead}: 線形射影 + MSE損失、
(d) \texttt{DIPHead}: 線形分類器 + BCE損失。
全パラメータが \texttt{torch.autograd} により自動微分可能となり、
NumPy 版の根本的制約（エンコーダ固定）が解消される。
Adam オプティマイザ（$\beta_1 = 0.9$, $\beta_2 = 0.999$, $\epsilon = 10^{-8}$）
とウォームアップ付きコサイン減衰スケジューリング~\cite{loshchilov2017sgdr}を採用する。

\textbf{Phase 2: 学習パイプライン（推定 1 週間）}。
(a) MTM 事前学習（100 エポック、マスク率 15\%）、
(b) DIP ファインチューニング（50 エポック、疑似ラベルによる教師あり学習）
の 2 段階学習パイプラインを構築する。
データ拡張として、アイテムのランダムドロップ（確率 10\%）、
時間軸のランダムクロッピング（75--100\% のサブシーケンス）、
密集区間のランダムシフト（$\pm 5$ 位置）を導入する。
検証指標として、MTM 損失の収束率、DIP AUC、
密集区間内外の埋め込み分離度を追跡する。

\textbf{Phase 3: スケーリングと評価（推定 2 週間）}。
(a) FlashAttention~\cite{dao2022flashattention}の導入により
$N > 10{,}000$ への適用を可能にする、
(b) 実データ（Online Retail: $N \approx 25{,}000$、
Dunnhumby: $N \approx 265{,}000$）での転移学習評価、
(c) 従来 Apriori-Window 法との精度・計算コストの定量比較を実施する。
