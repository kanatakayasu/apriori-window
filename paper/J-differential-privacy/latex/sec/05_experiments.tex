...
$\varepsilon = 0.1$（強いプライバシー保護）でもJaccard 0.429を達成しており、密集パターンの概形は捕捉されている。

\subsection{E2: メカニズム比較}

ラプラスメカニズム、ガウスメカニズム、SVTの3手法を$\varepsilon = 2.0$で比較した（表\ref{tab:e2}）。
...
\begin{tabular}{lrrr}
\toprule
メカニズム & Jaccard (平均) & Jaccard (標準偏差) & 実行時間 (ms) \\
\midrule
Laplace  & 0.348 & 0.083 & 0.31 \\
Gaussian & 0.311 & 0.049 & 0.37 \\
SVT      & 0.233 & 0.089 & 0.11 \\
\bottomrule
\end{tabular}
\end{table}

ラプラスメカニズムが最も高いJaccard（0.348）を達成した。
ガウスメカニズムはやや低いが標準偏差が小さく安定している。
SVTは最も高速であるが、ノイズ閾値との比較という性質上、区間境界の精度がやや低い。
...
実行時間は$n$に対してほぼ線形にスケールしており、$n = 2000$でも51 msと実用的な速度である。
DPのオーバーヘッド（ノイズ生成）は全体の実行時間に対して無視できる水準である。

\subsection{E4: 閾値安定性}

$\varepsilon$と信頼度$p$に対する安定性マージン$\gamma$を算出した（表\ref{tab:e4}）。

\begin{table}[t]
\centering
\caption{E4: 安定性マージン$\gamma$（$\Delta_W = 1$）}
\label{tab:e4}
\begin{tabular}{crrrr}
\toprule
...
クエリ数が少ない場合（$k = 10$）は高度合成の$\sqrt{k}$項のオーバーヘッドが支配的で逐次合成が有利であるが、$k \geq 50$では高度合成が優位となる。
$k = 500$で77.5\%の予算削減を達成しており、大規模データセットへの適用時に実質的な利点がある。
