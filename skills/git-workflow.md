...
同じ PR に複数コミットが含まれてよい。ただし 1 コミットに複数意図を混ぜない。
...
- **失敗例1: テスト失敗のままコミット** / 原因: push 前チェックを省略 / 回避策: § 3.2 のコマンドをコピペして必ず実行する
- **失敗例2: 複数意図が 1 コミットに混在** / 原因: `git add .` で全ファイルをまとめてステージ / 回避策: ファイルを 1 つずつ `git add <path>` する
- **失敗例3: main に直接コミット** / 原因: ブランチ切り忘れ / 回避策: 作業開始時に `git branch` で現在のブランチを確認する
- **失敗例4: PR 本文のテスト件数が実測と違う** / 原因: 推測で記入 / 回避策: `cargo test` / `pytest` を PR 作成直前に再実行して件数を確認する
- **失敗例5: force push で upstream を上書き** / 原因: rebase 後の push / 回避策: force push は絶対に使わない。rebase が必要な場合はユーザーに確認する
