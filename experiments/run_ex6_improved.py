"""
EX6-improved: Zipf-Distribution Robustness with relative magnitude scoring.

Compares baseline (absolute magnitude) vs improved (relative magnitude) configs
to measure the effect of use_effect_size and min_relative_change on Zipf data.
"""
import json
import sys
...
# --- Attribution configs to compare ---
ATTR_CONFIGS = {
    ...
    "relative_mag": lambda seed: AttributionConfig(
        min_support_range=5,
        n_permutations=5000,
        alpha=0.10,
        correction_method="bh",
    ...
    "relative_mag+filter0.3": lambda seed: AttributionConfig(
        min_support_range=5,
        n_permutations=5000,
        alpha=0.10,
        correction_method="bh",
    ...
    "relative_mag+filter0.5": lambda seed: AttributionConfig(
        min_support_range=5,
        n_permutations=5000,
        alpha=0.10,
        correction_method="bh",
    ...
    "relative_mag+filter1.0": lambda seed: AttributionConfig(
        min_support_range=5,
        n_permutations=5000,
        alpha=0.10,
        correction_method="bh",
...
def run_comparison():
    """Compare baseline vs improved configs across all Zipf conditions."""
    print("=" * 80)
    print("EX6-Improved: Zipf Robustness — Baseline vs Relative Magnitude")
    print("=" * 80)
    ...
    for cond_name, config_fn in CONDITIONS.items():
        ...
        # Generate data once per condition+seed (reuse across configs)
        data_cache = {}
    ...
    save_path = str(RESULTS_DIR / "ex6_improved_results.json")
    ...
    # Summary table
    print("\n" + "=" * 80)
    print(f"{'Condition':<12s} {'Config':<28s} {'P':>6s} {'R':>6s} {'F1':>6s} {'FAR':>6s}")
    print("-" * 80)
    for cond_name, cond_data in all_results.items():
        for attr_name, data in cond_data.items():
            print(f"{cond_name:<12s} {attr_name:<28s} "
