"""
Re-run EX1, EX2 (score ablation), EX2b (pipeline steps), EX2c (normalization ablation),
and EX3 (Dunnhumby) with magnitude_normalization="sqrt" as default.

Outputs a comprehensive summary for paper update.
"""
import json
import sys
import time
...
DATA_DIR = Path(__file__).resolve().parent / "data" / "sqrt_default"
N_SEEDS = 5

SQRT_CONFIG = dict(magnitude_normalization="sqrt")


def make_attr_config(seed, **overrides):
    """Standard attribution config with sqrt normalization."""
    cfg = dict(
        min_support_range=5,
        n_permutations=5000,
        alpha=0.10,
...
def run_ex1a():
    print("\n" + "=" * 60)
    print("EX1a: Beta Sweep (√norm default)")
    print("=" * 60)
    results = {}
    for beta in [0.1, 0.2, 0.3, 0.5]:
        results[f"beta_{beta}"] = run_condition(
            ...
            lambda seed: make_attr_config(seed),
        )
    return results


# =====================================================================
# EX1b: Structural conditions
# =====================================================================
def run_ex1b():
    print("\n" + "=" * 60)
    print("EX1b: Structural Conditions (√norm default)")
    print("=" * 60)
    CONDITIONS = {
...
# =====================================================================
# EX1c: Zipf conditions (uniform row only — Zipf rows already confirmed)
# =====================================================================
def run_ex1c_uniform():
    print("\n" + "=" * 60)
    print("EX1c: Uniform reference (√norm default)")
    print("=" * 60)
    return run_condition(
        "ex1c_uniform",
        lambda seed: make_ex1_config(boost=0.3, seed=seed),
        lambda seed: make_attr_config(seed),
    )


# =====================================================================
# EX2a: Score component ablation
# =====================================================================
def run_ex2a():
    print("\n" + "=" * 60)
    print("EX2a: Score Component Ablation (√norm default)")
    print("=" * 60)
    ...
    for sc_name, sc_fn in SCENARIOS.items():
        ...
        for abl_name, abl_mode in ABLATION_MODES.items():
            ...
            for seed in range(N_SEEDS):
                config = sc_fn(seed)
                out_dir = str(DATA_DIR / f"ex2a_{sc_name}_{abl_name}_seed{seed}")
                info = generate_synthetic(config, out_dir)
                attr_config = make_attr_config(seed, ablation_mode=abl_mode)
                result = run_single_experiment(
                    info["txn_path"], info["events_path"], info["gt_path"],
...
# =====================================================================
# EX2b: Pipeline step ablation
# =====================================================================
def run_ex2b():
    print("\n" + "=" * 60)
    print("EX2b: Pipeline Step Ablation (√norm default)")
    print("=" * 60)
    ...
    for cond_name, config_fn in CONDITIONS.items():
        ...
        for method_name, method_cfg in METHODS.items():
            ...
            for seed in range(N_SEEDS):
                config = config_fn(seed)
                out_dir = str(DATA_DIR / f"ex2b_{cond_name}_seed{seed}")
                info = generate_synthetic(config, out_dir)
...
# =====================================================================
# EX2c: Normalization ablation (uniform + Zipf, √norm vs none)
# =====================================================================
def run_ex2c():
    print("\n" + "=" * 60)
    print("EX2c: Normalization Ablation (√norm vs none)")
    print("=" * 60)

    CONDITIONS = {
        "uniform": lambda seed: make_ex1_config(boost=0.3, seed=seed),
    ...
    for cond_name, config_fn in CONDITIONS.items():
        cond_results = {}
        minsup = 5 if "zipf" in cond_name else 3

        for norm_name, norm_val in [("sqrt", "sqrt"), ("none", "none")]:
            seed_results = []
            for seed in range(N_SEEDS):
                config = config_fn(seed)
                out_dir = str(DATA_DIR / f"ex2c_{cond_name}_{norm_name}_seed{seed}")
                info = generate_synthetic(config, out_dir)
                attr_config = make_attr_config(seed, magnitude_normalization=norm_val)
                result = run_single_experiment(
                    info["txn_path"], info["events_path"], info["gt_path"],
                    window_size=50, min_support=minsup, max_length=100,
                    config=attr_config,
                    unrelated_path=info.get("unrelated_path"),
                )
                seed_results.append(asdict(result))
            avg = avg_metrics(seed_results)
            print(f"  {cond_name} / {norm_name}: P={avg['avg_precision']:.2f} "
                  f"R={avg['avg_recall']:.2f} F1={avg['avg_f1']:.2f} FAR={avg['avg_far']:.2f}")
            cond_results[norm_name] = {"seeds": seed_results, **avg}
        results[cond_name] = cond_results
    return results
...
if __name__ == "__main__":
    ...
    all_results["ex1a"] = run_ex1a()
    all_results["ex1b"] = run_ex1b()
    all_results["ex1c_uniform"] = run_ex1c_uniform()
    all_results["ex2a"] = run_ex2a()
    all_results["ex2b"] = run_ex2b()
    all_results["ex2c"] = run_ex2c()

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    with open(str(RESULTS_DIR / "all_sqrt_results.json"), "w") as f:
    ...
    print("\n--- EX1a: Beta Sweep ---")
    for k, v in all_results["ex1a"].items():
        print(f"  {k}: P={v['avg_precision']:.2f} R={v['avg_recall']:.2f} "
              f"F1={v['avg_f1']:.2f} FAR={v['avg_far']:.2f}")

    print("\n--- EX1b: Structural ---")
    for k, v in all_results["ex1b"].items():
        print(f"  {k}: P={v['avg_precision']:.2f} R={v['avg_recall']:.2f} "
              f"F1={v['avg_f1']:.2f} FAR={v['avg_far']:.2f}")

    print("\n--- EX1c: Uniform ref ---")
    v = all_results["ex1c_uniform"]
    print(f"  uniform: P={v['avg_precision']:.2f} R={v['avg_recall']:.2f} "
          f"F1={v['avg_f1']:.2f} FAR={v['avg_far']:.2f}")

    print("\n--- EX2a: Score Ablation ---")
    for sc, sc_data in all_results["ex2a"].items():
        for abl, v in sc_data.items():
            print(f"  {sc}/{abl}: F1={v['avg_f1']:.2f}")

    print("\n--- EX2b: Pipeline Steps ---")
    for cond, cond_data in all_results["ex2b"].items():
    ...
    print("\n--- EX2c: Normalization Ablation ---")
    for cond, cond_data in all_results["ex2c"].items():
        for norm, v in cond_data.items():
            print(f"  {cond}/{norm}: P={v['avg_precision']:.2f} R={v['avg_recall']:.2f} "
                  f"F1={v['avg_f1']:.2f} FAR={v['avg_far']:.2f}")

    print("\nDone. Results saved to", RESULTS_DIR / "all_sqrt_results.json")
